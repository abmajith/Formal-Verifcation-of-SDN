...
#define tot_iot_prt 4/*  total IoT ports available in the data plane */
...
int prt_IoT_dist[tot_OVS + 1] = {0,  2,  tot_iot_prt}; 
...
int status_iot_prt_dev_view[tot_iot_prt] = 0;/* these are the view of device of which Iot ports it got stored and when OVS forward based on these
                                                    information */
int status_iot_prt_ovs_view[tot_iot_prt] = 0;/* these are the OVS view about the status of the Iot ports for this case it wont matter but this array
                                                will be usefull if we go for the local controller and peripheral OVS*/
...
if 
:: ( Iot_no != 0 )  ->  status_iot_prt_dev_view[Iot_no-1] = 0 /*first it disconnect from the existing Iot port*/
:: else -> skip
fi;
do
                          ...
                          :: ( status_iot_prt_dev_view[cou-1] == 0 ) -> 
                                  ...
                                  :: ( cou <= prt_IoT_dist[cou1] ) ->
                                      OVS_que_data[cou1-1]!maclearning,IoT,cou,dev,0;/* here the count represents  IoT number */
                                          ...
                                          status_iot_prt_dev_view[Iot_no-1] = dev;/*IoT_no now occupied by the device dev */
                                                                    goto Lb
                                  :: (count > prt_IoT_dist[cou1] ) -> cou1 = cou1 + 1
                                  od
                          :: else ->   cou = cou + 1
                          od
od;
...
bool sec_mac_mes = false; /*to resent the mac message using different locations */
int x, z; /*to read the incoming ping messages  */
mtype y; /*to read the incoming ping messages */
end3:
do
:: ( ( sec_mac_mes == false ) && empty(dev_data_que[dev-1]) ) -> atomic {/* here device try to resend the mac message to the data plane possible in different positions*/
                        int count = 1, count1 = 1 ;/* count iterate over ovs number and its Iot port to join one of them non-deterministically */
                        do
                        :: (  status_iot_prt_dev_view[count-1] == 0  )  -> 
                                        do
                                        :: ( count <= prt_IoT_dist[count1] ) ->
                                                              OVS_que_data[count1-1]!maclearning,IoT,count,dev,0;/* here the count represents  IoT number */
                                                              lnk_OVS_no = count1;/* device i chose non deterministically the OVS count1 */
                                                              status_iot_prt_dev_view[Iot_no-1] = 0; /* first disconnect from previous positions */
                                                              Iot_no = count;/*now device connected to the IoT number count */
                                                              status_iot_prt_dev_view[Iot_no-1] = dev;/*IoT_no now occupied by the device dev */
                                                              goto L32
                                        :: (count > prt_IoT_dist[count1] ) -> count1 = count1 + 1
                                        od
                        :: ( status_iot_prt_dev_view[count-1] != 0 )  ->  count = count + 1
                        od;
                        L32: sec_mac_mes = true;
                        printf("\n device %d is connected to the OVS %d\n",dev,lnk_OVS_no);                        
                        }
:: dev_data_que[dev-1]?[y]  ->  atomic { dev_data_que[dev-1]?y,x,z; /* here device reading the incoming ping message */
