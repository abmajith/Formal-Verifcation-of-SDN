...
#define tot_OVS     3/* contains total number of OVS in the data plane topology*/
...
int c1 = 1;
 do
 :: ( c1 < no_device )  -> int c2 = c1 + 1, cntt = 1, ju = 1;
                    do
                ...
                Lq1:  /* I need to add the code for the other device first report to some IoT devices and check the rest of the formulae */
                      /* here i wrote only two domain only */
                    if
                ...
                Lq4:
                    ...
                    :: ( c2 <= no_device )  -> 
                                if
                                :: ( VS[c1-1] == VS[c2-1] )  ->  int dd = 1;
                                            do
                                            :: ( dd <= no_dom )  -> 
                                                                if
                                                                :: ( dd != ju)  ->  cont_que_ovs_rep[dd-1]!maclearning,Infra,no_OVS_dom[dd],1,c2; 
                                                                              if
...
/*To check the intermediate formula \phi1$$ */
int d1 = 1;
do
:: ( d1 < no_device ) -> int d2 = d1 + 1;
         ...
         LP1: 
              ...
              Lp2: printf("\n device %d connected in domain %d \n", d1, j);
                    printf("\n device %d connected in domain %d \n",d2, j1);
                   /* two  devices chose its random OVS initially*/
              do
